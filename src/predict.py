...
@click.option(
    ...
    type=click.BOOL,
    default=True,
    help="Save predictions along with the Ground Truth images if output-dir is not provided",
)
@click.option(
    "--csv-preds", type=click.STRING, default=None, help="CSV file to save the predictions paths"
)
@click.option(
    "--mc-ratio",
    type=click.FLOAT,
...
@click.option(
    "--mc-samples", type=click.INT, default=0, help="Number of Monte Carlo Dropout samples"
)
@click.option(
    "--predict-split", type=click.STRING, default="test", help="Split to use for prediction"
)
def predict(
    data_root,
    centers,
    ...
    dataloader = WMHDataModule(
        data_root, batch_size, centers, split_ratios, seed=seed, predict_split=predict_split
    )

    model = WMHModel.load_test(
        model_path, save_predictions, output_dir, patch_size, mc_ratio, mc_samples
    )
