...
MAIN_KEYS = {
    'data-root', 'centers', 'split-ratios', 'epochs', 'batch-size', 'lr',
    'dropout', 'loss', 'weight-decay', 'seed', 'patch-size',
    'samples-per-volume', 'queue-length', 'tio-num-workers',
    'reg-start', 'meep-lambda', 'ood-centers',
}

TEST_KEYS = {
    'data-root', 'centers', 'split-ratios', 'model-path',
    'batch-size', 'patch-size', 'seed',
}
...
def test_load_defaults_main_has_expected_keys():
    result = load_defaults('main')
    assert MAIN_KEYS.issubset(result.keys()), (
        f"Missing keys: {MAIN_KEYS - result.keys()}"
    )
...
def test_load_defaults_test_has_expected_keys():
    result = load_defaults('test')
    assert TEST_KEYS.issubset(result.keys()), (
        f"Missing keys: {TEST_KEYS - result.keys()}"
    )
...
def test_load_defaults_returns_empty_dict_when_mlproject_missing(tmp_path, mlproject_file):
    ...
    original = Path.__truediv__  # keep a reference
